...
mqttClient.on('message', async (topic, payload) => {
  ...
  try {
    ...
    const loteResult = await pool.query('SELECT lote_id FROM lotes WHERE lote_id = $1', [lote_id]);
    if (loteResult.rows.length === 0) {
      await pool.query('INSERT INTO lotes (lote_id, nome_lote) VALUES ($1, $2)', [lote_id, `Lote ${lote_id}`]);
    ...
    `;
    const values = [sensor_id, lote_id, temp_c, umidade_pct, bateria_pct, status];
    await pool.query(query, values);
    console.log('Dados inseridos no banco de dados com sucesso.');
  } catch (err) {
    console.error('Erro ao processar a mensagem MQTT e inserir no banco de dados:', err.message);
  }
});
